...
-- VERSION: 4
--
-- Sistema visual de progreso F10 con draws/markers dinamicos usando MIST.
--
-- CAMBIO VERSION 4:
-- - Se separan los colores de los QUAD/lineas de los colores de circulos/textos.
-- - COLORS controla circulos y textos.
-- - QUAD_COLORS controla solamente las conexiones tipo QUAD.
--
-- BASE VERSION 3:
-- - Las conexiones NO usan linea primitiva.
-- - Las conexiones se dibujan como QUAD delgado, mucho mas visible en F10.
-- - airports acepta formato:
--      ["Kenley"] = { bandera = 142, valor = nil }
-- - Se agrega airportOrder para mantener el orden visual.
-- - Cada aeropuerto puede tener:
--      Zone = "NombreZona"
--      zone = "NombreZona"
--      connectTo = "OtraBaseOZona"
--      target = "OtraBaseOZona"
--      links = { "Zona1", "OtraBase", "Zona2" }
-- - Si el destino existe como aeropuerto, conecta al aeropuerto.
-- - Si el destino existe como trigger zone, conecta a la zona.
--
-- CARGA RECOMENDADA:
-- 1) mist_4_5_128.lua
...
if not mist or not mist.marker or not mist.marker.add or not mist.marker.remove then
    ...
    if trigger and trigger.action and trigger.action.outText then
        trigger.action.outText("[HDEV_DPS] ERROR: MIST no esta cargado. Carga mist_4_5_128.lua antes de HDEV_DrawProgressSystem.lua", 15)
    end
...
-- ============================================================================
-- LIMPIEZA SI EL SCRIPT SE RECARGA EN CALIENTE
-- ============================================================================
...
DPS.CONFIG = {
    ...
    -- El script convierte internamente a 0-1.
    CONVERT_COLORS_TO_0_1 = true,

    -- ID alto para no chocar con otros marks.
    MARK_ID_START = 880000,

    -- ========================================================================
    -- COLORES PARA CIRCULOS Y TEXTOS
    -- ========================================================================
    ...
    -- ========================================================================
    -- COLORES SOLO PARA QUAD / CONEXIONES
    -- ========================================================================

    QUAD_COLORS = {
        [0] = {
            name = "QUAD_NEUTRAL",
            line = {160, 160, 160, 230},
...
--
-- airportOrder manda el orden visual.
--
-- airports usa formato compatible con tus tablas:
--      ["Kenley"] = { bandera = 142, valor = nil }
--
-- Zone / zone / connectTo / target / links:
-- sirven para crear conexiones extra desde ese aeropuerto hacia una zona trigger
-- o hacia otro aeropuerto.
-- ============================================================================
...
DPS.ROUTES = {
    {
        id = "AFGHANISTAN_MAIN",
        enabled = true,
        name = "Ruta principal Afganistán",
        coalition = -1,
        ...
        extraLineMode = "progress", -- "static" o "progress"
        extraLineWidth = 20,

        -- ESTA TABLA MANDA EL ORDEN.
        airportOrder = {
            "Jalalabad",
            "Bagram",
            "Kabul",
        ...
        -- ESTA TABLA ES COMPATIBLE CON TU FORMATO.
        airports = {
            ["Jalalabad"] = {
                bandera = 118,
                valor = nil,
                connectTo = "Kabul"
                -- Puede ser zona trigger o aeropuerto:
                -- Zone = "ZONA_KENLEY_FRONT"
                -- connectTo = "Kenley"
                -- links = {
                --     "ZONA_MANSTON_NORTE",
                --     "Hawkinge"
...
DPS.STATE = {
    ...
    nextMarkId = DPS.CONFIG.MARK_ID_START or 880000,
...
local function getLineColorForControls(c1, c2, route, forcedMode)
    c1 = normalizeControlValue(c1) or 0
    c2 = normalizeControlValue(c2) or 0

    local mode = lower(forcedMode or route.lineMode or "progress")
    local quadColors = DPS.CONFIG.QUAD_COLORS or DPS.CONFIG.COLORS

    if mode == "static" then
        local cs = quadColors.pending or quadColors[0]
        return cs.line, cs.fill, "PENDIENTE"
    end

    if c1 == 2 and c2 == 2 then
        local cs = quadColors[2]
        return cs.line, cs.fill, "AZUL"
    end

    if c1 == 1 and c2 == 1 then
        local cs = quadColors[1]
        return cs.line, cs.fill, "ROJO"
    end

    if c1 == 0 and c2 == 0 then
        local cs = quadColors[0] or quadColors.pending
        return cs.line, cs.fill, "NEUTRAL"
    end

    local cs = quadColors.mixed or quadColors.pending or quadColors[0]
    return cs.line, cs.fill, "MIXTO"
end
...
local function getNextMarkId()
    DPS.STATE.nextMarkId = (tonumber(DPS.STATE.nextMarkId) or tonumber(DPS.CONFIG.MARK_ID_START) or 880000) + 1
    return DPS.STATE.nextMarkId
end
...
local function scanAirports()
    ...
    if DPS.CONFIG.DEBUG then
        for _, data in ipairs(DPS.STATE.airportList) do
            env.info(
                "[HDEV_DPS] AIRPORT: " ..
                tostring(data.name) ..
                " | id=" .. tostring(data.id) ..
                " | point=" .. pointSnapshot(data.point)
            )
        end
    end
...
local function getRouteAirportNames(route)
    local names = {}

    if type(route.airportOrder) == "table" then
        for _, airportName in ipairs(route.airportOrder) do
            names[#names + 1] = tostring(airportName)
        end
    ...
    --     { name = "Herat", flag = 100 },
    -- }
    if type(route.airports) == "table" then
        for _, entry in ipairs(route.airports) do
    ...
    if #names == 0 and type(route.airports) == "table" then
        for k, _ in pairs(route.airports) do
        ...
        warn("Ruta " .. tostring(route.id) .. " no tiene airportOrder. Se uso orden alfabetico como fallback.")
    end

    return names
end

local function getRouteAirportEntry(route, airportName)
    if type(route.airports) ~= "table" then
        return {}
    end
    ...
    --     ["Kenley"] = { bandera = 142, valor = nil }
    -- }
    if type(route.airports[airportName]) == "table" then
        return route.airports[airportName]
    end

    local wanted = lower(airportName)

    for k, v in pairs(route.airports) do
        if type(k) == "string" and lower(k) == wanted and type(v) == "table" then
            return v
        end
    end
    ...
    -- }
    for _, entry in ipairs(route.airports) do
        if type(entry) == "table" and entry.name and lower(entry.name) == wanted then
            return entry
        end
    end
...
local function getEntryFlag(entry)
    if type(entry) ~= "table" then
        return nil
    end

    return entry.flag or entry.bandera
end
...
local function getAirportControl(route, airportEntry, airportData)
    ...
    if mode == "flag" then
        local val, source = readFlagValue(getEntryFlag(airportEntry))
    ...
    -- HYBRID:
    -- 1. controlAeropuertos[nombre]
    -- 2. coalicionPorBase[nombre]
    -- 3. estadoBanderasAeropuertos[nombre].valor
    -- 4. valor local route.airports[nombre].valor
    -- 5. bandera local route.airports[nombre].bandera / flag
    -- 6. estadoBanderasAeropuertos[nombre].bandera
    -- 7. control real
    local val, source = readGlobalTableValue("controlAeropuertos", airportName)
    ...
    if airportEntry.valor ~= nil then
        val = normalizeControlValue(airportEntry.valor)

        if val ~= nil then
            return val, "route.airports[" .. tostring(airportName) .. "].valor"
        end
    end

    val, source = readFlagValue(getEntryFlag(airportEntry))
...
-- ============================================================================
-- POINT RESOLVER PARA AIRPORT / ZONE
-- ============================================================================

local function resolveNamedPoint(name)
    if not name then
        return nil, "nil", nil
    end
    ...
    if airportData and airportData.point then
        return normalizePoint(airportData.point), "airport", airportData
    end
    ...
    if okZone and zone and zone.point then
        return normalizePoint(zone.point), "zone", zone
    end

    return nil, "missing", nil
end

local function collectExtraLinks(entry)
    local links = {}
    local seen = {}

    local function addLink(v)
    ...
    addLink(entry.connectTo)
    addLink(entry.ConnectTo)
    addLink(entry.target)
    addLink(entry.Target)

    if type(entry.links) == "table" then
        for _, v in ipairs(entry.links) do
            addLink(v)
        end
    end
...
local function makeLineQuadPoints(p1, p2, width)
    ...
    local half = safeNumber(width, DPS.CONFIG.DEFAULT_LINE_WIDTH) / 2
    ...
    return {
        { x = p1.x + nx, y = p1.y, z = p1.z + nz },
        { x = p2.x + nx, y = p2.y, z = p2.z + nz },
        { x = p2.x - nx, y = p2.y, z = p2.z - nz },
        { x = p1.x - nx, y = p1.y, z = p1.z - nz }
    }
end

local function addQuadLine(markerKey, p1, p2, color, fillColor, lineType, coa, message, width, snapshot)
    if DPS.STATE.markerSnapshots[markerKey] == snapshot and DPS.STATE.markers[markerKey] then
        return getMarkerId(markerKey), false
    end
    ...
    local quad = makeLineQuadPoints(p1, p2, width)

    if not quad then
        warn("No se pudo crear QUAD line: " .. tostring(markerKey) .. " | puntos invalidos")
        return nil, false
    end
    ...
    if ok then
        ...
        debugLog("Linea QUAD creada: " .. markerKey .. " | markId=" .. tostring(markId))
        return markId, true
    end

    warn("Error creando linea QUAD: " .. tostring(markerKey) .. " | " .. tostring(err))
    return nil, false
end
...
local function buildLineName(route, index)
    return "DPS_" .. safeName(route.id) .. "_LINE_" .. tostring(index)
end

local function buildExtraLineName(route, index, targetName, linkIndex)
    return "DPS_" .. safeName(route.id) .. "_EXTRA_" .. tostring(index) .. "_" .. tostring(linkIndex) .. "_" .. safeName(targetName)
end
...
local function drawMainLine(route, runtimeA, runtimeB, lineIndex)
    if not runtimeA or not runtimeB then
        debugLog("Linea saltada por runtime faltante en ruta " .. tostring(route.id) .. " index=" .. tostring(lineIndex))
        return
    end

    if not runtimeA.found or not runtimeB.found or not runtimeA.data or not runtimeB.data then
        debugLog("Linea saltada por aeropuerto faltante en ruta " .. tostring(route.id) .. " index=" .. tostring(lineIndex))
        return
    end
    ...
    if not p1 or not p2 then
        debugLog("Linea saltada por punto invalido en ruta " .. tostring(route.id) .. " index=" .. tostring(lineIndex))
        return
    end

    local lineColor, lineFill, lineState = getLineColorForControls(runtimeA.control, runtimeB.control, route, route.lineMode)
    local markerKey = buildLineName(route, lineIndex)

    local width = safeNumber(route.lineWidth, DPS.CONFIG.DEFAULT_LINE_WIDTH)
...
local function drawExtraLinks(route, runtimeEntry)
    if not runtimeEntry or not runtimeEntry.found or not runtimeEntry.data then
        return
    end

    local links = collectExtraLinks(runtimeEntry.entry)

    if #links == 0 then
        return
    end

    local p1 = normalizePoint(runtimeEntry.data.point)

    if not p1 then
        return
    end

    for linkIndex, targetName in ipairs(links) do
        local p2, targetType, targetObj = resolveNamedPoint(targetName)

        if not p2 then
            debugLog(
                "Extra link no encontrado: " ..
                tostring(runtimeEntry.data.name) ..
                " -> " ..
                tostring(targetName)
            )
        else
            local targetControl = runtimeEntry.control

            if targetType == "airport" and targetObj then
                local targetEntry = getRouteAirportEntry(route, targetObj.name)
                targetControl = select(1, getAirportControl(route, targetEntry or {}, targetObj))
            end

            local lineColor, lineFill, lineState = getLineColorForControls(
                runtimeEntry.control,
                targetControl,
                route,
                route.extraLineMode or route.lineMode
            )

            local markerKey = buildExtraLineName(route, runtimeEntry.index, targetName, linkIndex)
            local width = safeNumber(route.extraLineWidth or route.lineWidth, DPS.CONFIG.DEFAULT_LINE_WIDTH)

            local snapshot =
                "extra_quad|" ..
                tostring(runtimeEntry.index) .. "|" ..
                tostring(linkIndex) .. "|" ..
                tostring(runtimeEntry.data.name) .. ">" .. tostring(targetName) .. "|" ..
                tostring(targetType) .. "|" ..
                pointSnapshot(p1) .. ">" .. pointSnapshot(p2) .. "|" ..
                tostring(runtimeEntry.control) .. ">" .. tostring(targetControl) .. "|" ..
                tostring(lineState) .. "|" ..
                tostring(route.lineType or DPS.CONFIG.DEFAULT_LINE_TYPE) .. "|" ..
                tostring(width)
...
local function buildRouteRuntime(route)
    local runtime = {}
    local names = getRouteAirportNames(route)

    for index, airportName in ipairs(names) do
        local airportEntry = getRouteAirportEntry(route, airportName)
        local airportData = findAirportByName(airportName)

        airportEntry.name = airportName
        ...
        if airportData then
            ...
            debugLog(
                "Ruta " .. tostring(route.id) ..
                " | aeropuerto NO encontrado: " .. tostring(airportName)
            )
        end
    end
...
local function buildRouteSnapshot(route, runtime, names)
    local parts = {}
    ...
    parts[#parts + 1] = "lineWidth=" .. tostring(route.lineWidth)
    parts[#parts + 1] = "extraLineMode=" .. tostring(route.extraLineMode)
    parts[#parts + 1] = "extraLineWidth=" .. tostring(route.extraLineWidth)
    ...
    for i = 1, #(names or {}) do
        ...
        if item and item.found and item.data then
            ...
            local links = collectExtraLinks(item.entry)

            for linkIndex, targetName in ipairs(links) do
                local p2, targetType = resolveNamedPoint(targetName)
...
local function drawRoute(route, force)
    ...
    local runtime, names = buildRouteRuntime(route)
    local routeSnapshot = buildRouteSnapshot(route, runtime, names)
    ...
    -- Circulos, textos y conexiones extra.
    for i = 1, #names do
        local item = runtime[i]
        ...
        if item and item.found and item.data then
            ...
            drawText(route, item)
            drawExtraLinks(route, item)
        end
    end

    -- Lineas principales entre aeropuerto i y aeropuerto i+1.
    for i = 1, #names - 1 do
        drawMainLine(route, runtime[i], runtime[i + 1], i)
    end
    ...
    for _, route in ipairs(DPS.ROUTES or {}) do
        ...
        if route.enabled == false then
            ...
            for i = 1, #names do
                ...
                if item and item.found and item.data then
                    local flagTxt = getEntryFlag(item.entry) and (" | flag=" .. tostring(getEntryFlag(item.entry))) or ""
                    ...
                    for _, targetName in ipairs(links) do
                        local _, targetType = resolveNamedPoint(targetName)

                        lines[#lines + 1] =
                            "     -> " .. tostring(targetName) .. " [" .. tostring(targetType) .. "]"
                    end
                else
                    lines[#lines + 1] =
                        "  " .. tostring(i) ..
