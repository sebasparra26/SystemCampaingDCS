...
----------------------------------------------------------------
-- ActivateUnitsCampaing_NORMANDIA.lua
-- VERSION: 5
--
-- BASE:
...
--   Logs internos corregidos de KOLA a NORMANDIA.
--
-- MODULO 1:
...
local WATCHTOWER_CONFIG = {
    ...
    RED_COUNTRY_ID = country.id.THIRDREICH,
...
local ACTSYNC = {
    ...
    FILE_PATH = getWriteDir() .. "Config\\HorizontDev\\NORMANDIA\\ActivateUnitsFlagPersistence.json",
...
local function log(msg)
    env.info("[ActivateUnitsCampaing_NORMANDIA] " .. tostring(msg))
    if debugActivo then
...
local function spawnWatchTowerForBase(entry, coalitionSide)
    ...
    if ok and result then
        ...
        if spawnedGroup then
            ...
            env.info("[ActivateUnitsCampaing_NORMANDIA] Watchtower terrestre creada base=" .. tostring(entry.name) .. " side=" .. tostring(coalitionSide) .. " group=" .. runtimeName .. " type=" .. tostring(WATCHTOWER_CONFIG.UNIT_TYPE))
            return true
        end
...
local function buildDocFromState()
    ...
    return {
        ...
        meta = {
            ...
            source = "ActivateUnitsCampaing_NORMANDIA",
...
local function activarGrupoOriginalPersistente(nombreGrupo, bandera)
    ...
    if not grp then
        ...
        env.info("[ActivateUnitsCampaing_NORMANDIA] No existe grupo en ME: " .. tostring(nombreGrupo))
        return false
    end
    ...
    if ok then
        ...
        env.info("[ActivateUnitsCampaing_NORMANDIA] Grupo activado por flag " .. tostring(bandera) .. ": " .. tostring(nombreGrupo))
        return true
    end

    debug("ERROR activando grupo '" .. tostring(nombreGrupo) .. "' por flag " .. tostring(bandera), 10)
    env.info("[ActivateUnitsCampaing_NORMANDIA] ERROR activando grupo '" .. tostring(nombreGrupo) .. "' por flag " .. tostring(bandera) .. ": " .. tostring(err))
    return false
end
