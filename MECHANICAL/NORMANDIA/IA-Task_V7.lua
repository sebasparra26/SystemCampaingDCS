...
-- - VERSION 13: AUTO RED CAS usa triple disparador: hook de setUserFlag + heartbeat + timer directo.
...
--
-- Objetivo:
-- - NO depende del menu F10.
-- - Lanza CAS rojo automaticamente cuando detecta aeropuertos en valor 2.
-- - Usa el mismo motor createTask() que ya funciona con el CAS manual.
--
-- Disparadores redundantes:
-- 1) Hook de trigger.action.setUserFlag(): si una bandera del rango pasa a 2,
--    intenta lanzar/revisar inmediatamente.
-- 2) Heartbeat principal del script.
-- 3) Timer directo de DCS como respaldo.
----------------------------------------------------------------
local AUTO_RED_CAS = AUTO_RED_CAS or {
    ...
    -- Ataque exitoso: espera 10 min para otro CAS.
    INTERVAL_SECONDS = 60 * 60,

    -- Primer chequeo automatico.
    START_DELAY = 20,

    -- Si no hay objetivo o falla algo, revisa mas rapido.
    RETRY_SECONDS = 45,
    ...
    -- NO necesita menu. Esto solo oculta o muestra comandos de diagnostico.
    SHOW_DEBUG_MENU = false,

    -- Triple respaldo de arranque.
    USE_FLAG_HOOK = true,
    USE_HEARTBEAT = true,
    USE_TIMER_DIRECT = true,
    TIMER_PULSE_SECONDS = 15,

    -- Para no spamear intentos en el mismo segundo si varias banderas cambian juntas.
...
local function autoRedCasLog(msg, seconds)
    ...
    if AUTO_RED_CAS.DEBUG then
        trigger.action.outText("[AUTO RED CAS] " .. tostring(msg), seconds or 8)
    end
end
...
local function autoRedCasSeedRandom()
    -- DCS puede cargar el entorno de scripting con math.randomseed sanitizado/nil.
    -- No lo usamos: math.random() ya existe en el entorno donde funciono el disparo manual.
    -- Si randomseed no existe y se llama, el modulo automatico muere antes de lanzar.
...
local function autoRedCasFindAirbasePointByName(airbaseName)
    ...
    if Airbase and Airbase.getByName then
        ...
        if okAB and ab and ab.getPoint then
...
local function autoRedCasAirportIsTarget(airportName, info)
    ...
    -- Respaldos: algunos sistemas ya tienen controlAeropuertos / coalicionPorBase actualizado
    -- aunque la bandera todavia no haya sido leida como numero por este script.
...
local function autoRedCasLaunch()
    ...
    if ok then
        ...
        autoRedCasLog(
            "CAS rojo lanzado contra: " .. tostring(selected.name) ..
...
local function autoRedCasShowStatus()
    ...
    local lines = {
        ...
        "Siguiente chequeo t: " .. tostring(math.floor(AUTO_RED_CAS_STATE.nextCheckAt or 0)),
...
local function autoRedCasPulse(source, force)
    ...
    if not AUTO_RED_CAS_STATE.armed then
        ...
        autoRedCasLog("Armado automatico por " .. tostring(source or "unknown") .. ". Primer chequeo en " .. tostring(AUTO_RED_CAS.START_DELAY or 20) .. " s.", 8)

        if not force then
            return false
        end

        -- Si el llamado viene forzado desde carga o cambio de bandera,
        -- no esperamos START_DELAY: revisamos inmediatamente.
    ...
    if launched then
        AUTO_RED_CAS_STATE.nextCheckAt = now + (AUTO_RED_CAS.INTERVAL_SECONDS or 600)
    else
...
local function autoRedCasFlagInRange(flag)
    local n = tonumber(flag)
    return n and n >= AUTO_RED_CAS.FLAG_MIN and n <= AUTO_RED_CAS.FLAG_MAX
end
...
local function autoRedCasInstallFlagHook()
    ...
    trigger.action.setUserFlag = function(flag, value)
        ...
        if nFlag and nValue
            ...
            -- No se fuerza a romper el intervalo de 10 min si ya hubo lanzamiento.
            -- Pero si el sistema aun no esta armado, lo arma y adelanta el primer chequeo.
            if not AUTO_RED_CAS_STATE.armed then
    ...
    autoRedCasLog("Hook setUserFlag instalado para banderas " .. tostring(AUTO_RED_CAS.FLAG_MIN) .. "-" .. tostring(AUTO_RED_CAS.FLAG_MAX) .. ".", 8)
end
...
if AUTO_RED_CAS.DEBUG then
    trigger.action.outText(
        "[AUTO RED CAS] V14 cargado. Automatico por hook + heartbeat + timer. No requiere F10.",
        12
    )
end


-- Primer intento inmediato. Si las tablas/banderas ya existen al cargar,
-- lanza sin esperar al timer ni al heartbeat.
...
trigger.action.outText(
    "Tasking IA V14 cargado. AUTO RED CAS automatico activo por hook/heartbeat/timer.",
