...
local AUTO_RED_CAS = AUTO_RED_CAS or {
    ...
    -- Cada cuanto intenta lanzar un ataque CAS rojo.
...
local function autoRedCasTick(_, now)
    autoRedCasLaunch()
    return now + AUTO_RED_CAS.INTERVAL_SECONDS
end

timer.scheduleFunction(function(_, now)
    if AUTO_RED_CAS_STATE.started then
        return nil
    end

    AUTO_RED_CAS_STATE.started = true

    math.randomseed(os.time() + math.floor(timer.getAbsTime() or 0))

    autoRedCasLog(
        "Modulo iniciado. Intervalo: " ..
        tostring(AUTO_RED_CAS.INTERVAL_SECONDS) .. " segundos."
    )

    autoRedCasLaunch()
    return now + AUTO_RED_CAS.INTERVAL_SECONDS
end, nil, timer.getTime() + AUTO_RED_CAS.START_DELAY)
...
trigger.action.outText(
    "Tasking IA V8 cargado. AUTO RED CAS disponible.",
