...
local function cobrarTankerRojo(tp)
  ...
  local ok, saldo = Economy.spend(side, costo, "Despliegue tanker rojo: " .. tostring(tp))
  return ok, saldo, costo
end

local function reembolsarTankerRojo(tp, costo)
  if not USAR_ECONOMIA or not REEMBOLSAR_SI_FALLA_SPAWN then
    return
  end

  Economy.add(side, costo or 0, "Reembolso tanker rojo fallido: " .. tostring(tp))
end

local function construirNombreGrupoRojo(tp)
  local intento = 0

  while intento < 50 do
    local nombre = tp:gsub("%s", "") .. "_" .. tostring(randChan()) .. "_RED"
  ...
  return tp:gsub("%s", "") .. "_" .. tostring(math.floor(timer.getTime())) .. "_RED"
end
...
local function spawnTankerRojo(tp, p1, p2, hdg)
  ...
  local okCobro, saldoDespues, costo = cobrarTankerRojo(tp)
  if not okCobro then
  ...
  local chan = randChan()
  local gName = construirNombreGrupoRojo(tp)
  ...
  if not okSpawn or not grp or not grp:isExist() then
    reembolsarTankerRojo(tp, costo)
    env.info("[TANKER RED] Error al crear tanker " .. tostring(tp) .. ": " .. tostring(errSpawn))
...
function eventHandlerRojo:onEvent(e)
  if e.id ~= world.event.S_EVENT_MARK_CHANGE or not e.text or not _G.__SEL_ROJO then
    return
  end
...
missionCommands.addCommandForCoalition(side, "Tanqueros Activos", rootMenu, function()
  ...
  if USAR_ECONOMIA then
    msg = msg .. "\nSaldo Rojo: " .. formatearDolaresLegible(saldoActual())
  end
