...
csar.maxLives = 50 -- Maximum pilot lives
...
-- ***************************************************************
-- **************** HDEV CSAR ECONOMY + PERSISTENCE **************
-- ***************************************************************
-- Pago por rescate confirmado en MASH/FARP/Airbase.
csar.hdevRescuePaymentEnabled = true
csar.hdevPaymentPerRescuedPilot = 500000      -- pago normal por piloto jugador
csar.hdevPaymentPerRescuedAI = 250000         -- pago por piloto IA si hdevPaymentPayAI = true
csar.hdevPaymentPayAI = true
csar.hdevPaymentCoalitionMode = "rescuer"     -- "rescuer" = coalicion del heli CSAR | "pilot" = coalicion del piloto caido

-- Persistencia JSON de pilotos caidos.
csar.hdevPersistenceEnabled = true
csar.hdevPersistenceFile = ((lfs and lfs.writedir and lfs.writedir()) or "") .. "Config\\HorizontDev\\AFGHANISTAN\\CSAR_Pilots.json"
csar.hdevPersistenceAutosaveInterval = 10
csar.hdevPersistenceMinWriteInterval = 3
csar.hdevRestoreInTransitAsDowned = true       -- si reinicia con piloto abordo, lo devuelve al terreno para no perderlo
csar.hdevPersistenceDebug = true
...
csar.hdevPersistence = csar.hdevPersistence or {
    initialized = false,
    loaded = false,
    dirty = false,
    lastWriteTime = 0,
...
local function hdevCsarLog(msg)
    env.info("[HDEV_CSAR] " .. tostring(msg))
    if csar.hdevPersistenceDebug then
        trigger.action.outText("[HDEV_CSAR] " .. tostring(msg), 8)
    end
end

local function hdevCsarWarn(msg)
    env.info("[HDEV_CSAR] " .. tostring(msg))
end

local function hdevNow()
    if timer and timer.getAbsTime then
        return math.floor(timer.getAbsTime())
    elseif timer and timer.getTime then
        return math.floor(timer.getTime())
    end
    return 0
end

local function hdevJsonEscape(str)
    str = tostring(str or "")
    str = str:gsub("\\", "\\\\")
    str = str:gsub('"', '\\"')
    str = str:gsub("\r", "\\r")
...
local function hdevSortedKeys(tbl)
    local keys = {}
...
local function hdevIsArray(tbl)
    ...
    return count == maxIndex and count > 0
end

local function hdevEncodeJsonValue(value, indent)
    indent = indent or 0
    ...
    elseif t == "string" then
        return '"' .. hdevJsonEscape(value) .. '"'
    elseif t ~= "table" then
        return '"' .. hdevJsonEscape(tostring(value)) .. '"'
    end
    ...
    if hdevIsArray(value) then
        local lines = {"["}
        for i = 1, #value do
            local comma = (i < #value) and "," or ""
            lines[#lines + 1] = string.rep(" ", indent + 2) .. hdevEncodeJsonValue(value[i], indent + 2) .. comma
        end
        lines[#lines + 1] = pad .. "]"
        return table.concat(lines, "\n")
    end

    local keys = hdevSortedKeys(value)
    local lines = {"{"}
    for i, key in ipairs(keys) do
        local comma = (i < #keys) and "," or ""
        lines[#lines + 1] = string.rep(" ", indent + 2) .. '"' .. hdevJsonEscape(tostring(key)) .. '": ' .. hdevEncodeJsonValue(value[key], indent + 2) .. comma
    end
    lines[#lines + 1] = pad .. "}"
    return table.concat(lines, "\n")
end

local function hdevEnsureDirectoryForFile(path)
...
local function hdevReadFile(path)
...
local function hdevWriteFile(path, txt)
    hdevEnsureDirectoryForFile(path)
    local f, err = io.open(path, "w")
    if not f then
        hdevCsarWarn("No se pudo escribir JSON CSAR: " .. tostring(err))
        return false
    end
    f:write(txt or "")
...
local function hdevDecodeJson(txt)
...
local function hdevFormatMoney(value)
...
local function hdevSafePoint(p)
    p = p or {}
    return {
        x = tonumber(p.x) or 0,
        y = tonumber(p.y) or 0,
...
local function hdevGetGroupPoint(groupName)
    local grp = Group.getByName(groupName)
    if grp and grp:isExist() and grp:getUnit(1) then
        return grp:getUnit(1):getPoint()
    end
    return nil
end

local function hdevGetCountryFromGroup(groupName, fallback)
    local grp = Group.getByName(groupName)
    if grp and grp:isExist() and grp:getUnit(1) then
        local ok, cid = pcall(function() return grp:getUnit(1):getCountry() end)
        if ok and cid then return cid end
    end
    return fallback
end

function csar.hdevMarkPersistenceDirty()
    if not csar.hdevPersistenceEnabled then return end
    csar.hdevPersistence.dirty = true
end

function csar.hdevBuildPilotRecord(groupName, groupInfo, status, point)
    groupInfo = groupInfo or {}
    local currentPoint = point or hdevGetGroupPoint(groupName) or groupInfo.point or { x = groupInfo.x, y = groupInfo.y, z = groupInfo.z }
    currentPoint = hdevSafePoint(currentPoint)

    local existing = csar.hdevPersistence.pilots[groupName] or {}
    local now = hdevNow()

    return {
        groupName = groupName,
        unitName = existing.unitName or groupInfo.unitName,
        status = status or existing.status or "downed",
        side = tonumber(groupInfo.side or groupInfo.coalition or existing.side) or 2,
        coalition = tonumber(groupInfo.side or groupInfo.coalition or existing.coalition) or 2,
        country = tonumber(groupInfo.country or existing.country or hdevGetCountryFromGroup(groupName, nil)) or ((tonumber(groupInfo.side or existing.side) == 1) and country.id.RUSSIA or country.id.USA),
        x = currentPoint.x,
        y = currentPoint.y,
        z = currentPoint.z,
        originalUnit = groupInfo.originalUnit or existing.originalUnit,
        desc = groupInfo.desc or existing.desc or tostring(groupName),
        typename = groupInfo.typename or groupInfo.typeName or existing.typename or existing.typeName,
        frequency = tonumber(groupInfo.frequency or existing.frequency),
        player = groupInfo.player or existing.player,
        rescueHeli = groupInfo.rescueHeli or existing.rescueHeli,
        createdAt = existing.createdAt or now,
        updatedAt = now
    }
end

function csar.hdevRegisterDownedPilot(groupName, groupInfo, spawnedGroup)
    if not csar.hdevPersistenceEnabled or not groupName then return end
    local point = nil
    if spawnedGroup and spawnedGroup:getUnit(1) then
        point = spawnedGroup:getUnit(1):getPoint()
    end
    csar.hdevPersistence.pilots[groupName] = csar.hdevBuildPilotRecord(groupName, groupInfo, "downed", point)
    csar.hdevMarkPersistenceDirty()
    csar.hdevSavePersistence(true)
end

function csar.hdevSetPilotInTransit(groupName, heliUnit, groupInfo, pickupPoint)
    if not csar.hdevPersistenceEnabled or not groupName then return end
    groupInfo = groupInfo or csar.woundedGroups[groupName] or csar.hdevPersistence.pilots[groupName] or {}
    if heliUnit then
        groupInfo.rescueHeli = heliUnit:getName()
        pickupPoint = pickupPoint or heliUnit:getPoint()
    end
    local rec = csar.hdevBuildPilotRecord(groupName, groupInfo, "inTransit", pickupPoint)
    rec.rescueHeli = groupInfo.rescueHeli
    csar.hdevPersistence.pilots[groupName] = rec
    csar.hdevMarkPersistenceDirty()
    csar.hdevSavePersistence(true)
end

function csar.hdevRemovePilotFromPersistence(groupName, reason)
    if not csar.hdevPersistenceEnabled or not groupName then return end
    if csar.hdevPersistence.pilots[groupName] then
        csar.hdevPersistence.pilots[groupName] = nil
        csar.hdevMarkPersistenceDirty()
        csar.hdevSavePersistence(true)
        hdevCsarLog("Piloto removido del JSON: " .. tostring(groupName) .. " | " .. tostring(reason or "sin motivo"))
    end
end

function csar.hdevCollectPersistencePilots()
    local pilots = {}

    for groupName, groupInfo in pairs(csar.woundedGroups or {}) do
        local grp = Group.getByName(groupName)
        if grp and grp:isExist() and grp:getUnit(1) then
            pilots[groupName] = csar.hdevBuildPilotRecord(groupName, groupInfo, "downed", grp:getUnit(1):getPoint())
        end
    end

    for heliName, groups in pairs(csar.inTransitGroups or {}) do
        local heli = Unit.getByName(heliName)
        for groupName, groupInfo in pairs(groups or {}) do
            groupInfo = groupInfo or {}
            groupInfo.rescueHeli = heliName
            local point = nil
            if heli and heli:isExist() then
                point = heli:getPoint()
            else
                local old = csar.hdevPersistence.pilots[groupName]
                if old then point = { x = old.x, y = old.y, z = old.z } end
            end
            pilots[groupName] = csar.hdevBuildPilotRecord(groupName, groupInfo, "inTransit", point)
        end
    end
...
function csar.hdevSavePersistence(force)
    if not csar.hdevPersistenceEnabled then return false end
    if not io or not io.open then
        hdevCsarWarn("io.open no disponible. No se puede guardar persistencia CSAR.")
        return false
    end

    local now = timer and timer.getTime and timer.getTime() or 0
    if not force then
        if not csar.hdevPersistence.dirty then return false end
        if (now - (csar.hdevPersistence.lastWriteTime or 0)) < (csar.hdevPersistenceMinWriteInterval or 3) then
            return false
        end
    end

    csar.hdevPersistence.pilots = csar.hdevCollectPersistencePilots()

    local payloadTable = {
        version = "HDEV_CSAR_1.0.0",
        updatedBy = "DCS",
        updatedAt = hdevNow(),
        restoreInTransitAsDowned = csar.hdevRestoreInTransitAsDowned and true or false,
        pilots = csar.hdevPersistence.pilots
    }

    local payload = hdevEncodeJsonValue(payloadTable, 0)
    if not force and payload == csar.hdevPersistence.lastSavedPayload then
        csar.hdevPersistence.dirty = false
        return false
    end

    local ok = hdevWriteFile(csar.hdevPersistenceFile, payload)
    if ok then
        csar.hdevPersistence.lastSavedPayload = payload
        csar.hdevPersistence.lastWriteTime = now
        csar.hdevPersistence.dirty = false
    end
    return ok
end

function csar.hdevReserveFrequency(freq)
    freq = tonumber(freq)
    if not freq or not csar.freeVHFFrequencies then return end
    for i, current in ipairs(csar.freeVHFFrequencies) do
        if tonumber(current) == freq then
            table.remove(csar.freeVHFFrequencies, i)
            break
        end
    end
    csar.usedVHFFrequencies = csar.usedVHFFrequencies or {}
    table.insert(csar.usedVHFFrequencies, freq)
end

function csar.hdevSpawnPersistedPilot(groupName, record)
    if type(record) ~= "table" then return false end

    local status = tostring(record.status or "downed")
    if status == "rescued" or status == "kia" then
        return false
    end
    if status == "inTransit" and not csar.hdevRestoreInTransitAsDowned then
        return false
    end

    local point = hdevSafePoint({ x = record.x, y = record.y, z = record.z })
    if point.x == 0 and point.z == 0 then
        return false
    end

    local finalGroupName = tostring(record.groupName or groupName)
    if Group.getByName(finalGroupName) and Group.getByName(finalGroupName):isExist() then
        return false
    end

    local side = tonumber(record.side or record.coalition) or 2
    local countryId = tonumber(record.country) or ((side == 1) and country.id.RUSSIA or country.id.USA)
    local freq = tonumber(record.frequency)
    if not freq then
        freq = csar.generateADFFrequency()
    else
        csar.hdevReserveFrequency(freq)
    end

    local spawnedGroup = csar.spawnGroup(side, countryId, point, record.typename or record.typeName, finalGroupName, record.unitName)
    if not spawnedGroup then
        hdevCsarWarn("No se pudo recrear piloto persistido: " .. tostring(finalGroupName))
        return false
    end

    csar.addSpecialParametersToGroup(spawnedGroup)
    csar.addBeaconToGroup(spawnedGroup:getName(), freq)

    local desc = record.desc
    if not desc or desc == "" then
        if record.player then
            desc = "Pilot " .. tostring(record.player) .. " of " .. tostring(record.originalUnit or "UNKNOWN") .. " - " .. tostring(record.typename or "UNKNOWN")
        else
            desc = "Persisted CSAR Pilot - " .. tostring(record.typename or "UNKNOWN")
        end
    end

    csar.woundedGroups[spawnedGroup:getName()] = {
        side = spawnedGroup:getCoalition(),
        originalUnit = record.originalUnit,
        desc = desc,
        typename = record.typename or record.typeName,
        frequency = freq,
    ...
    csar.hdevPersistence.pilots[spawnedGroup:getName()] = csar.hdevBuildPilotRecord(spawnedGroup:getName(), csar.woundedGroups[spawnedGroup:getName()], "downed", point)
    csar.initSARForPilot(spawnedGroup, freq, true)

    local controller = spawnedGroup:getController()
    if controller then
        Controller.setOption(controller, AI.Option.Ground.id.ALARM_STATE, AI.Option.Ground.val.ALARM_STATE.GREEN)
        Controller.setOption(controller, AI.Option.Ground.id.ROE, AI.Option.Ground.val.ROE.WEAPON_HOLD)
    end

    hdevCsarLog("Piloto CSAR restaurado desde JSON: " .. tostring(spawnedGroup:getName()))
    return true
end

function csar.hdevLoadPersistence()
    if not csar.hdevPersistenceEnabled then return end
    if not io or not io.open then
        hdevCsarWarn("io.open no disponible. No se puede leer persistencia CSAR.")
        return
    end

    local txt = hdevReadFile(csar.hdevPersistenceFile)
    if not txt then
        csar.hdevPersistence.loaded = true
        csar.hdevSavePersistence(true)
        return
    end

    local data, err = hdevDecodeJson(txt)
    if not data then
        hdevCsarWarn("No se pudo leer JSON CSAR: " .. tostring(err))
        csar.hdevPersistence.loaded = true
        return
    end

    local restored = 0
    csar.hdevPersistence.pilots = {}
    for groupName, record in pairs(data.pilots or {}) do
        if csar.hdevSpawnPersistedPilot(groupName, record) then
            restored = restored + 1
        end
    end
    ...
    csar.hdevMarkPersistenceDirty()
    csar.hdevSavePersistence(true)
    hdevCsarLog("Persistencia CSAR cargada. Pilotos restaurados: " .. tostring(restored))
end

function csar.hdevGetRescuePaymentAmount(rescueGroup)
    rescueGroup = rescueGroup or {}
    if rescueGroup.player == nil or rescueGroup.player == "" then
        if not csar.hdevPaymentPayAI then return 0 end
        return tonumber(csar.hdevPaymentPerRescuedAI) or 0
    end
    return tonumber(csar.hdevPaymentPerRescuedPilot) or 0
end

function csar.hdevPayRescue(heliUnit, rescueGroup)
    if not csar.hdevRescuePaymentEnabled then return false end
    if not heliUnit or not rescueGroup then return false end

    local amount = csar.hdevGetRescuePaymentAmount(rescueGroup)
    if amount <= 0 then return false end

    local payCoalition = heliUnit:getCoalition()
    if csar.hdevPaymentCoalitionMode == "pilot" then
        payCoalition = tonumber(rescueGroup.side) or payCoalition
    end
    if payCoalition ~= 1 and payCoalition ~= 2 then
        payCoalition = heliUnit:getCoalition()
    end

    if not HDEV_Economy or not HDEV_Economy.add then
        hdevCsarWarn("Rescate confirmado, pero HDEV_Economy no esta disponible. No se pago: " .. tostring(rescueGroup.desc))
        return false
    end

    local before = HDEV_Economy.get and HDEV_Economy.get(payCoalition) or 0
    local after = HDEV_Economy.add(payCoalition, amount, "CSAR rescate: " .. tostring(rescueGroup.desc or rescueGroup.woundedGroup or "piloto"))

    env.info("[HDEV_CSAR_PAYMENT] coalicion=" .. tostring(payCoalition) .. " monto=" .. tostring(amount) .. " piloto=" .. tostring(rescueGroup.desc) .. " heli=" .. tostring(heliUnit:getName()) .. " saldoAntes=" .. tostring(before) .. " saldoDespues=" .. tostring(after))

    trigger.action.outTextForCoalition(
        payCoalition,
        "Pago CSAR recibido\n" ..
        "Helicoptero: " .. tostring(heliUnit:getName()) .. "\n" ..
        "Rescate: " .. tostring(rescueGroup.desc or rescueGroup.woundedGroup or "piloto") .. "\n" ..
        "Valor: " .. hdevFormatMoney(amount) .. "\n" ..
        "Saldo: " .. hdevFormatMoney(after or 0),
        12
    )
...
function csar.hdevPersistenceTick()
    if csar.hdevPersistenceEnabled then
        csar.hdevSavePersistence(false)
        return timer.getTime() + (csar.hdevPersistenceAutosaveInterval or 10)
    end
    return nil
end

function csar.hdevInitPersistence()
    if csar.hdevPersistence.initialized then return end
    csar.hdevPersistence.initialized = true
    if not csar.hdevPersistenceEnabled then return end
    csar.hdevLoadPersistence()
    timer.scheduleFunction(csar.hdevPersistenceTick, nil, timer.getTime() + (csar.hdevPersistenceAutosaveInterval or 10))
end
...
function csar.addCsar(_coalition , _country, _point, _typeName, _unitName, _playerName, _freq, noMessage, _description )
  ...
  -- HDEV FIX: el CSAR original enviaba _playerName a handleEjectOrCrash,
  -- pero esa funcion espera un objeto Unit. Esto evita error si usas csarMode 1/2/3.
  local _originalUnitObject = nil
  if _unitName ~= nil then
      _originalUnitObject = Unit.getByName(_unitName)
  end
  if _originalUnitObject ~= nil then
      csar.handleEjectOrCrash(_originalUnitObject, false)
  end
...
-- Generate DESCRIPTION text
  ...
  csar.woundedGroups[_spawnedGroup:getName()] = { side = _spawnedGroup:getCoalition(), originalUnit = _unitName, desc = _text, typename = _typeName, frequency = _freq, player = _playerName, country = _country }
  csar.hdevRegisterDownedPilot(_spawnedGroup:getName(), csar.woundedGroups[_spawnedGroup:getName()], _spawnedGroup)
...
function csar.spawnGroup( _coalition, _country, _point, _typeName, _persistedGroupName, _persistedUnitName )

    local _id = mist.getNextGroupId()

    local _groupName = _persistedGroupName or ("Downed Pilot #" .. _id)
   ...
   local _side = _coalition
    ...
    if _side == 2 then
        _group.units[1] = csar.createUnit(_pos.x + 50, _pos.z + 50, 120, "Soldier M4", _persistedUnitName)
    else
        _group.units[1] = csar.createUnit(_pos.x + 50, _pos.z + 50, 120, "Infantry AK", _persistedUnitName)
    end
...
function csar.createUnit(_x, _y, _heading, _type, _persistedUnitName)

    local _id = mist.getNextUnitId();

    local _name = _persistedUnitName or string.format("Wounded Pilot #%s", _id)
...
function csar.checkWoundedGroupStatus(_argument)
    local _status, _err = pcall(function(_args)
        ...
        -- HDEV: evita error nil cuando el grupo ya fue subido al heli o murio.
        if #_woundedGroup == 0 then
            if _heliUnit ~= nil then
                csar.checkGroupNotKIA(_woundedGroup, _woundedGroupName, _heliUnit, _heliName)
            end
            return
        end
...
function csar.pickupUnit(_heliUnit, _pilotName, _woundedGroup, _woundedGroupName)
    ...
    csar.inTransitGroups[_heliName][_woundedGroupName] =
    {
        ...
        typename = csar.woundedGroups[_woundedGroupName].typename,
        frequency = csar.woundedGroups[_woundedGroupName].frequency,
        country = csar.woundedGroups[_woundedGroupName].country,
    }

    csar.hdevSetPilotInTransit(_woundedGroupName, _heliUnit, csar.inTransitGroups[_heliName][_woundedGroupName], _woundedLeader:getPoint())
...
function csar.checkGroupNotKIA(_woundedGroup, _woundedGroupName, _heliUnit, _heliName)
    ...
    if #_woundedGroup == 0 and _heliUnit ~= nil then
        ...
        if inTransit == false then
            ...
            csar.hdevRemovePilotFromPersistence(_woundedGroupName, "KIA")
        else
            csar.hdevSavePersistence(true)
        end
...
function csar.rescuePilots(_heliUnit)
    ...
    local _paymentCount = 0

    -- enable pilots again + HDEV payment/persistence cleanup
    for _woundedGroupName, _rescueGroup in pairs(_rescuedGroups) do

        csar.enableAircraft(_rescueGroup.originalUnit, _rescueGroup.player)
        if csar.hdevPayRescue(_heliUnit, _rescueGroup) then
            _paymentCount = _paymentCount + 1
        end
        csar.hdevRemovePilotFromPersistence(_rescueGroup.woundedGroup or _woundedGroupName, "RESCUED")
    end

    if _paymentCount > 0 then
        _txt = _txt .. "\nPagos CSAR confirmados: " .. tostring(_paymentCount)
    end
...
function csar.addMedevacMenuItem()
    ...
    for key, unitName in pairs(csar.csarFixedUnits) do
      if csar.csarUnits[unitName] == nil then
        ...
        local _fixedUnit = csar.getSARHeli(unitName)
        if _fixedUnit ~= nil then
          for _woundedName, _groupInfo in pairs(csar.woundedGroups) do
            if _groupInfo.side == _fixedUnit:getCoalition() then
              -- Schedule timer to check when to pop smoke
...
-- HDEV: cargar pilotos persistidos despues de preparar frecuencias ADF.
csar.hdevInitPersistence()
