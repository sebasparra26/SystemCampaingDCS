local WHSYNC = {
    ...
    FILE_PATH = lfs.writedir() .. "Config\\HorizontDev\\SystemWarehousesPersistanceSinai.json",
...
local WAREHOUSE_SETTING_METHODS = {
    unlimitedAircrafts = {
        setters = { "setUnlimitedAircrafts", "setUnlimitedAircraft", "setUnlimitedPlanes" },
        getters = { "getUnlimitedAircrafts", "isUnlimitedAircrafts", "getUnlimitedAircraft", "isUnlimitedAircraft" }
    },
    unlimitedMunitions = {
        setters = { "setUnlimitedMunitions", "setUnlimitedAmmo", "setUnlimitedWeapons" },
        getters = { "getUnlimitedMunitions", "isUnlimitedMunitions", "getUnlimitedAmmo", "isUnlimitedAmmo" }
    },
    unlimitedFuel = {
        setters = { "setUnlimitedFuel" },
        getters = { "getUnlimitedFuel", "isUnlimitedFuel" }
    },
    dynamicCargo = {
        setters = { "setDynamicCargo", "setDynamicCargoEnabled" },
        getters = { "getDynamicCargo", "isDynamicCargo", "getDynamicCargoEnabled", "isDynamicCargoEnabled" }
    },
    dynamicSpawn = {
        setters = { "setDynamicSpawn", "setDynamicSpawnEnabled" },
        getters = { "getDynamicSpawn", "isDynamicSpawn", "getDynamicSpawnEnabled", "isDynamicSpawnEnabled" }
    },
    allowHotStart = {
        setters = { "setAllowHotStart", "setHotStart", "setHotStartAllowed" },
        getters = { "getAllowHotStart", "isAllowHotStart", "getHotStart", "isHotStartAllowed" }
    },
    coalition = {
        setters = { "setCoalition" },
        getters = { "getCoalition" }
    }
}
...
local function tryWarehouseCall(wh, methodName, arg)
    if not wh or not methodName then
        return false, nil
    end
    local fn = wh[methodName]
...
local function getWarehouseSettingSnapshot(wh)
    local settings = {}

    for key, def in pairs(WAREHOUSE_SETTING_METHODS) do
        for _, getter in ipairs(def.getters or {}) do
            local ok, result = tryWarehouseCall(wh, getter, nil)
            if ok then
                if key == "coalition" then
                    settings[key] = tonumber(result) or result
                else
                    local b = normalizeBool(result)
                    if b ~= nil then
                        settings[key] = b
                    else
                        settings[key] = result
...
local function applySettingsMap(wh, settings)
    if type(settings) ~= "table" then
        return
    end

    for key, value in pairs(settings) do
        local def = WAREHOUSE_SETTING_METHODS[key]
        if def then
            local writeValue = value

            if key ~= "coalition" then
                local b = normalizeBool(value)
                if b ~= nil then
                    writeValue = b
                end
            else
            ...
            local applied = false
            for _, setter in ipairs(def.setters or {}) do
                local ok = tryWarehouseCall(wh, setter, writeValue)
                if ok then
                    applied = true
                    if WHSYNC.DEBUG then
                        log("Setting aplicado: " .. tostring(key) .. " = " .. tostring(writeValue))
                    end
                    break
                end
            end

            if (not applied) and WHSYNC.DEBUG then
                log("Setting no soportado por warehouse runtime: " .. tostring(key))
            end
        end
    end
end
...
local function snapshotAirbase(id, ab)
    ...
    local snap = {
        ...
        settings = getWarehouseSettingSnapshot(wh),
    }
...
local function applyLocationData(ab, data)
    ...
    if data.settings then
        applySettingsMap(wh, data.settings)
    end
end
...
local function exportLiveToJson()
    ...
    for id, ab in pairs(WHSTATE.airbasesById) do
        ...
        if snap then
            ...
            elseif snap.settings then
                doc[section][tostring(id)].settings = snap.settings
            end
        end
    end
