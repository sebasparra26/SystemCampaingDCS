...
NAVP.CONFIG = {
    ...
    FILE_PATH = lfs.writedir() .. "Config\\HorizontDev\\SystemUnitPositionPersistence.json",
    ...
    PRESERVE_ROUTE = true,
    DESTROY_RUNTIME_IF_EXISTS_ON_FIRST_INJECT = true,

    GROUPS = {
        {
            key = "Marshall",
            templateGroupName = "Marshall",
            runtimeGroupName = "Marshall",
            enabled = true
        },
        {
        key = "Tarawa",
        templateGroupName = "Tarawa",
        runtimeGroupName = "Tarawa",
        enabled = true
    }
    }
}
...
local function destroyRuntimeGroup(name)
...
local function buildDefaultEntryFromTemplate(cfg)
    ...
    local entry = {
        ...
        runtimeGroupName = cfg.runtimeGroupName,
        alive = true,
...
local function buildDefaultDocument()
    local doc = {
        ...
        meta = {
            ...
            note = "Grupo naval Late Activation usado como plantilla; runtime group persistido por JSON"
        },
        groups = {}
    }
...
local function mergeConfiguredGroupsIntoDoc(doc)
    ...
    for _, cfg in ipairs(NAVP.CONFIG.GROUPS or {}) do
        if not doc.groups[cfg.key] then
            ...
            doc.groups[cfg.key].runtimeGroupName = cfg.runtimeGroupName
...
local function shiftRoute(route, dx, dy)
    if not route or not route.points then return end

    for _, pt in ipairs(route.points) do
        if pt.x ~= nil then pt.x = (tonumber(pt.x) or 0) + dx end
        if pt.y ~= nil then pt.y = (tonumber(pt.y) or 0) + dy end
        if pt.point and type(pt.point) == "table" then
            if pt.point.x ~= nil then pt.point.x = (tonumber(pt.point.x) or 0) + dx end
            if pt.point.y ~= nil then pt.point.y = (tonumber(pt.point.y) or 0) + dy end
        end
    end
end

local function buildRuntimeGroupData(cfg, entry)
    local template = getTemplateData(cfg.templateGroupName)
    if not template then
        return nil, "No existe plantilla naval"
    end

    local data = deepCopy(template)
    local lead = template.units[1]

    local targetAnchorX = tonumber(entry.anchor.mapPoint.x) or tonumber(lead.x) or 0
    local targetAnchorY = tonumber(entry.anchor.mapPoint.y) or tonumber(lead.y) or 0

    local dx = targetAnchorX - (tonumber(lead.x) or 0)
    local dy = targetAnchorY - (tonumber(lead.y) or 0)

    data.name = cfg.runtimeGroupName
    data.groupName = cfg.runtimeGroupName
    data.groupId = mist.getNextGroupId()
    data.category = "ship"

    if NAVP.CONFIG.PRESERVE_ROUTE and data.route then
        shiftRoute(data.route, dx, dy)
    end

    for i, unit in ipairs(data.units or {}) do
        local savedUnit = entry.units and entry.units[i] or nil

        local targetX = (tonumber(unit.x) or 0) + dx
        local targetY = (tonumber(unit.y) or 0) + dy

        if savedUnit and savedUnit.mapPoint then
            targetX = tonumber(savedUnit.mapPoint.x) or targetX
            targetY = tonumber(savedUnit.mapPoint.y) or targetY
        end

        unit.x = targetX
        unit.y = targetY
        unit.point = { x = targetX, y = targetY }
        unit.unitId = mist.getNextUnitId()

        if savedUnit and savedUnit.heading then
            unit.heading = tonumber(savedUnit.heading) or unit.heading
        end

        local unitName = cfg.runtimeGroupName .. "_U" .. string.format("%02d", i)
        unit.name = unitName
...
local function injectEntry(cfg, entry)
    ...
    if groupExistsByName(cfg.runtimeGroupName) then
        return true, "ya existe runtime"
    end

    if not NAVP.STATE.firstInjectCycleDone and NAVP.CONFIG.DESTROY_RUNTIME_IF_EXISTS_ON_FIRST_INJECT then
        destroyRuntimeGroup(cfg.runtimeGroupName)
    end

    local groupData, err = buildRuntimeGroupData(cfg, entry)
    ...
    if not ok or not result then
        return false, result or "mist.dynAdd devolvio nil"
    end
    ...
    for _, cfg in ipairs(NAVP.CONFIG.GROUPS or {}) do
        ...
        if entry and not NAVP.STATE.injectDoneByKey[cfg.key] then
            ...
            if ok then
                ...
                log("Grupo naval clonado: " .. tostring(cfg.runtimeGroupName))
            else
                log("Error clonando grupo naval: " .. tostring(res))
            end
        end
    end
...
local function exportLiveToJson()
    ...
    doc.meta.source = "DCS Naval runtime"
    doc.meta.note = "El runtime ship group es el persistido"
