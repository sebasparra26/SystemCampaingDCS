...
--
-- CAMBIO VERSION 17:
-- - Base: VERSION 13 estable.
-- - Agrega tabla editable por aeropuerto para cambiar el helicoptero de entrega.
-- - No toca menu.
-- - No toca core.
-- - No toca economia.
-- - No toca aterrizaje ni Controller.setTask().
--
-- CARGAR RECOMENDADO:
-- 1) mist_4_5_128.lua
...
-- ============================================================================
-- FILTRO MANUAL DE HELICOPTERO POR AEROPUERTO
-- ============================================================================
-- AQUI EDITAS TU.
--
-- Si un aeropuerto NO esta en esta tabla:
--   BLUE usa AR.CONFIG.BLUE_TRANSPORT_TYPE = "CH-47Fbl1"
--   RED  usa AR.CONFIG.RED_TRANSPORT_TYPE  = "Mi-8MT"
--
-- Si un aeropuerto SI esta en esta tabla:
--   puedes cambiar el helicoptero que llega a ese aeropuerto.
--
-- IMPORTANTE:
-- - El nombre del aeropuerto debe ser EXACTO como aparece en DCS.
-- - Esto NO cambia el menu.
-- - Esto NO cambia la ruta.
-- - Esto solo cambia la aeronave que se crea para ese destino.
--
-- FORMATO SIMPLE:
-- ["Nombre Aeropuerto"] = {
--     BLUE = "UH-1H",
...
HDEV_MARKETPLACE_AIRPORT_TRANSPORT_OVERRIDES = {
    -- EJEMPLOS:
     ["Qala i Naw"] = {
         BLUE = "UH-1H",
         RED  = "Mi-24P",
...
local function resolveTransportForAirport(cfg, airportName, data)
    local side = tonumber(cfg and cfg.coalition) or 0

    local defaultType = data.transportType or (
        side == 2 and AR.CONFIG.BLUE_TRANSPORT_TYPE or AR.CONFIG.RED_TRANSPORT_TYPE
    )

    local defaultCountryId = data.countryId or (
        side == 2 and AR.CONFIG.BLUE_COUNTRY_ID or AR.CONFIG.RED_COUNTRY_ID
    )

    local defaultSpeed = tonumber(data.velocidad or cfg.defaultSpeed or AR.CONFIG.DEFAULT_SPEED) or 42
    local defaultSpawnAltAGL = tonumber(data.spawnAltAGL or AR.CONFIG.SPAWN_ALT_AGL) or 250

    local result = {
        type = defaultType,
        countryId = defaultCountryId,
        speed = defaultSpeed,
        spawnAltAGL = defaultSpawnAltAGL,
        overrideUsed = false,
        overrideSource = nil,
    }

    local overrides = HDEV_MARKETPLACE_AIRPORT_TRANSPORT_OVERRIDES or {}
    local airportOverride = overrides[airportName]

    if type(airportOverride) ~= "table" then
        return result
    end

    local sideKey = side == 2 and "BLUE" or "RED"
    local override = airportOverride[sideKey] or airportOverride[side]

    if type(override) == "string" then
        result.type = override
        result.overrideUsed = true
        result.overrideSource = airportName .. "." .. sideKey
        return result
    end

    if type(override) == "table" then
        result.type = override.type or override.transportType or result.type
        result.countryId = override.countryId or override.country or result.countryId
        result.speed = tonumber(override.speed or override.velocidad or result.speed) or result.speed
        result.spawnAltAGL = tonumber(override.spawnAltAGL or override.altAGL or result.spawnAltAGL) or result.spawnAltAGL
        result.overrideUsed = true
        result.overrideSource = airportName .. "." .. sideKey
        return result
    end
...
function AR.rebuildGlobalRouteTables()
    ...
    for index, airport in ipairs(airports) do
        ...
        newPlantillasB[name] = {
            autoRoute = true,
            template = "AUTO_BLUE_" .. safeName(name),
            bandera = flagB,
        ...
        newPlantillasR[name] = {
            autoRoute = true,
            template = "AUTO_RED_" .. safeName(name),
            bandera = flagR,
            origen = deepCopy(spawn),
            velocidad = tonumber(AR.CONFIG.DEFAULT_SPEED) or 42,
            airportName = name,
...
local function inspectLiveGroup(groupName)
    ...
    if not grp then
        out.reason = "Group.getByName nil"
        return out
    end
    ...
    out.exists = okExist and exists or false

    if not out.exists then
        out.reason = "group no existe"
        return out
    end

    out.groupId = grp:getID()
    out.category = grp:getCategory()
    out.coalition = grp:getCoalition()
    out.units = {}
    ...
    for i, unit in ipairs(units) do
        ...
        out.units[i] = {
            ...
            velocity = {
                ...
                y = v.y,
                z = v.z,
                speed2D = math.sqrt((v.x or 0)^2 + (v.z or 0)^2),
            }
        }
    end
...
local function createDynamicHelicopter(cfg, airport, subKey, data, purchaseControl)
    ...
    local transportCfg = resolveTransportForAirport(cfg, airport, data)

    local speed = tonumber(transportCfg.speed) or 42
    local transportType = transportCfg.type
    local countryId = transportCfg.countryId
    local spawnAltAGL = tonumber(transportCfg.spawnAltAGL) or tonumber(AR.CONFIG.SPAWN_ALT_AGL) or 250

    data.transportType = transportType
    data.countryId = countryId
    data.velocidad = speed
    data.spawnAltAGL = spawnAltAGL
    ...
    local spawnAltMSL = originGroundMSL + spawnAltAGL
    ...
    local groupData = {
        ...
        __HDEV_DEBUG = {
            ...
            transportOverrideUsed = transportCfg.overrideUsed,
            transportOverrideSource = transportCfg.overrideSource,
            spawnAltAGL = spawnAltAGL,
        }
    }
...
function AR.install()
    ...
    log("Instalado VERSION 17. Base estable VERSION 13 + filtro manual de helicoptero por aeropuerto.")
    return true
end
