...
MS.CONFIG = MS.CONFIG or {
    ...
    FILE_PATH = lfs.writedir() .. "Config\\HorizontDev\\SystemMissionPersistence.json",
...
local function evaluateSecondaryObjectiveMonitor(mState, objectiveDef)
    ...
    if kind == "group" then
        ...
        if mon.requireExists == true and not data.exists then
            return false, 0, data
        end

        local metricName = mon.metric or "aliveUnits"
        local metricValue = ensureNumber(data[metricName])
        return compareValues(metricValue, mon.op or "==", mon.value or 0), metricValue, data
    ...
    elseif kind == "flag" then
        ...
        return compareValues(current, mon.op or "==", mon.value or 1), current, { flag = mon.flag }
    ...
    elseif kind == "warehouse" then
        ...
        if mon.itemName then
            ...
            return memory.removedCount >= memory.targetRemoved, memory.removedCount, memory
        end
        ...
        for i = 1, #items do
            ...
            itemMemory.removedCount = math.max(0, ensureNumber(itemMemory.initialCount) - ensureNumber(currentCount))
        ...
        if mode == "any" then
            return anyPass, totalRemoved, memory
        elseif mode == "sum" then
            return totalRemoved >= memory.targetRemovedTotal, totalRemoved, memory
        else
            return allPass, totalRemoved, memory
        end
    end
