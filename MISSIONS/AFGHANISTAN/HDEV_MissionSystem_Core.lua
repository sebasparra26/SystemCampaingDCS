...
MS.CONFIG = MS.CONFIG or {
    ...
    rootRelativePath = "Scripts\\HorizontDev\\AFGHANISTAN\\",
    jsonRelativePath = "Config\\HorizontDev\\AFGHANISTAN\\SystemMissionPersistenceAfghanistan.json",
    ...
    warehouseJsonRelativePaths = {
        "Config\\HorizontDev\\AFGHANISTAN\\SistemWarehousePersistanceAfghanistan.json",
        "Config\\HorizontDev\\AFGHANISTAN\\SistemWarehousePersistanceAfghanistan.json",
    },
...
local function getMissionJsonPath()
    return buildWriteDirPath(MS.CONFIG.jsonRelativePath or "Config\\HorizontDev\\KOLA\\SystemMissionPersistenceKola.json")
end
...
local function ensureSecondaryObjectiveState(mState, objectiveId)
    ...
    if not mState.secondaryObjectivesState[objectiveId] then
        mState.secondaryObjectivesState[objectiveId] = {
            ...
            cleanupDueAt = nil,
            cleanupDelaySeconds = nil,
            cleanupExecuted = false,
            cleanupExecutedAt = nil,
            cleanupResult = nil,
...
-- JSON SAVE / RESTORE
----------------------------------------------------------------
local function buildObjectiveDefinitionSnapshot(objectiveDef)
    local mon = objectiveDef and objectiveDef.monitor or {}
    ...
    return {
        id = objectiveDef and objectiveDef.id or nil,
        name = objectiveDef and objectiveDef.name or nil,
        drawName = objectiveDef and objectiveDef.drawName or nil,
        enabled = objectiveDef and (objectiveDef.enabled ~= false) or false,
        requiredForMission = objectiveDef and (objectiveDef.requiredForMission == true) or false,

        targetName = targetName,
        reward = deepCopy(objectiveDef and objectiveDef.reward or {}),
        ...
        monitor = {
            ...
            baseName = mon.baseName,
            airbaseName = mon.airbaseName,
            zoneName = mon.zoneName,
            coalition = mon.coalition,

            groupMode = mon.groupMode,
            mode = mon.mode,

            itemName = mon.itemName,
            items = deepCopy(mon.items or {}),

            point = deepCopy(mon.point),
            radius = mon.radius
        },

        setFlagOnPass = deepCopy(objectiveDef and objectiveDef.setFlagOnPass or {}),
        setFlagOnActive = deepCopy(objectiveDef and objectiveDef.setFlagOnActive or {})
    }
end

local function buildSecondaryObjectivesCatalog(def)
    local catalog = {}

    for _, objectiveDef in ipairs(def.secondaryObjectives or {}) do
        local objectiveId = tostring(objectiveDef.id or "")
        if objectiveId ~= "" then
            catalog[objectiveId] = buildObjectiveDefinitionSnapshot(objectiveDef)
        end
    end
...
local function buildSaveDocument()
    ...
    for _, def in ipairs(sortedMissionDefs()) do
        local mState = ensureMissionState(def)
        local savedSecondaryState = deepCopy(mState.secondaryObjectivesState or {})

        for _, objectiveDef in ipairs(def.secondaryObjectives or {}) do
            local objectiveId = tostring(objectiveDef.id or "")
            if objectiveId ~= "" then
                savedSecondaryState[objectiveId] = savedSecondaryState[objectiveId] or {}
                savedSecondaryState[objectiveId].definitionSnapshot = buildObjectiveDefinitionSnapshot(objectiveDef)
            end
        end
        ...
        doc.missionsState[def.id] = {
            ...
            secondaryObjectivesCatalog = buildSecondaryObjectivesCatalog(def),
            secondaryObjectivesState = savedSecondaryState
        }
    end
...
local function resolveMonitorZone(zoneName, zonePoint, zoneRadius)
    local zone = nil

    if zoneName and zoneName ~= "" then
        if mist and mist.DBs and mist.DBs.zonesByName and mist.DBs.zonesByName[zoneName] then
            zone = deepCopy(mist.DBs.zonesByName[zoneName])
        end

        if not zone then
            local rawZone = trigger.misc.getZone(zoneName)
            if rawZone then
                zone = {
                    name = zoneName,
                    radius = rawZone.radius,
                    point = nil,
                    x = rawZone.x,
                    y = rawZone.y
                }

                if rawZone.point then
                    zone.point = {
                        x = ensureNumber(rawZone.point.x),
                        y = ensureNumber(rawZone.point.y),
                        z = rawZone.point.z ~= nil and ensureNumber(rawZone.point.z) or ensureNumber(rawZone.point.y)
                    }
                else
                    zone.point = {
                        x = ensureNumber(rawZone.x),
                        y = 0,
                        z = ensureNumber(rawZone.y)
                    }
                end
            end
        end
    elseif type(zonePoint) == "table" then
        zone = {
            name = nil,
            radius = ensureNumber(zoneRadius),
            point = {
                x = ensureNumber(zonePoint.x),
                y = ensureNumber(zonePoint.y),
                z = zonePoint.z ~= nil and ensureNumber(zonePoint.z) or ensureNumber(zonePoint.y)
            }
        }
    end
...
local function pointInsideMonitorZone(point, zone)
    if not point or not zone then
        return false, nil
    end

    local verts = zone.verticies or zone.vertices or zone.verts
    if type(verts) == "table" and #verts >= 3 and mist and mist.pointInPolygon then
        return mist.pointInPolygon(point, verts) == true, nil
    end

    local center = zone.point or {
        x = ensureNumber(zone.x),
        y = 0,
        z = ensureNumber(zone.y)
    }

    local dx = ensureNumber(point.x) - ensureNumber(center.x)
    local dz = ensureNumber(point.z ~= nil and point.z or point.y) - ensureNumber(center.z ~= nil and center.z or center.y)
    local dist = math.sqrt((dx * dx) + (dz * dz))
    local radius = ensureNumber(zone.radius)
...
local function getUnitAliveInZoneMetrics(unitName, zoneName, zonePoint, zoneRadius)
    local out = {
        exists = false,
        alive = 0,
        life = 0,
        ...
        zoneExists = false,
        zoneName = zoneName,
        zoneRadius = ensureNumber(zoneRadius),
        distance2D = -1,
        unitName = unitName
    }

    local zone = resolveMonitorZone(zoneName, zonePoint, zoneRadius)
    if zone then
        out.zoneExists = true
        out.zoneName = tostring(zone.name or zoneName or "PUNTO")
        out.zoneRadius = ensureNumber(zone.radius)
        out.zonePoint = {
            x = ensureNumber(zone.point and zone.point.x or zone.x),
            y = ensureNumber(zone.point and zone.point.y or 0),
            z = ensureNumber((zone.point and (zone.point.z ~= nil and zone.point.z or zone.point.y)) or zone.y)
        }
    end
    ...
    if point then
        out.point = {
            ...
            z = ensureNumber(point.z)
        }
    end

    if out.zoneExists and out.point then
        local inZone, distance2D = pointInsideMonitorZone(out.point, zone)
        out.inZone = inZone and true or false
        if distance2D ~= nil then
            out.distance2D = ensureNumber(distance2D)
        end
    end
...
local function getGroupAliveInZoneMetrics(groupName, zoneName, zonePoint, zoneRadius, groupMode)
    local out = {
        exists = false,
        alive = 0,
        totalUnits = 0,
        ...
        distance2D = -1,
        groupName = groupName,
        groupMode = tostring(groupMode or "any"):lower()
    }
    ...
    for i = 1, #units do
        ...
        if unit and unit:isExist() then
            ...
            if point then
                ...
                if out.zoneExists then
                    local inZone, distance2D = pointInsideMonitorZone(pointData, zone)
                    if distance2D ~= nil then
                        if nearestDist == nil or distance2D < nearestDist then
                            nearestDist = distance2D
                        end
                    end
                    if alive and inZone == true then
                        out.unitsInZoneAlive = out.unitsInZoneAlive + 1
                    end
                end
            end
        end
    ...
    if nearestDist ~= nil then
        out.distance2D = ensureNumber(nearestDist)
    end

    if out.groupMode == "all" then
        out.inZone = (out.aliveUnits > 0) and (out.unitsInZoneAlive >= out.aliveUnits)
        out.alive = out.inZone and 1 or 0
    elseif out.groupMode == "count" then
        out.inZone = out.unitsInZoneAlive > 0
        out.alive = out.unitsInZoneAlive
    else
        out.inZone = out.unitsInZoneAlive > 0
        out.alive = out.inZone and 1 or 0
    end
...
local function normalizeSmokeColorId(color)
    if type(color) == "number" then
        local n = math.floor(color)
        if n >= 0 and n <= 4 then
            return n
        end
    end

    local s = tostring(color or "white"):lower()
    if s == "green" or s == "verde" then return 0 end
    if s == "red" or s == "rojo" then return 1 end
    if s == "white" or s == "blanco" then return 2 end
    if s == "orange" or s == "naranja" then return 3 end
    if s == "blue" or s == "azul" then return 4 end
    return 2
end

local function getUnitWorldPointByName(unitName)
    local unit = unitName and Unit.getByName(unitName) or nil
    if not unit or not unit:isExist() then
        return nil
    end
...
local function getGroupWorldPointByName(groupName)
    local grp = groupExistsByName(groupName)
    if not grp then
        return nil
    end
    ...
    for i = 1, #units do
        local unit = units[i]
        if unit and unit:isExist() and ensureNumber(unit:getLife()) > 0 then
            local point = unit:getPoint()
            if point then
                return {
                    x = ensureNumber(point.x),
...
local function getAirbaseWorldPointByName(baseName)
    local base = baseName and Airbase.getByName(baseName) or nil
    if not base then
        return nil
    end
...
local function getObjectiveSmokeConfig(objectiveDef)
    local mon = objectiveDef and objectiveDef.monitor or nil
    if not mon then
        return nil
    end

    local smoke = mon.smoke or objectiveDef.smoke
    if type(smoke) ~= "table" or smoke.enabled ~= true then
        return nil
    end
...
local function getObjectiveSmokeState(missionId, objectiveId)
    MS.STATE.runtimeObjectiveSmokes[missionId] = MS.STATE.runtimeObjectiveSmokes[missionId] or {}
    MS.STATE.runtimeObjectiveSmokes[missionId][objectiveId] = MS.STATE.runtimeObjectiveSmokes[missionId][objectiveId] or {
        lastRefreshAt = -9999
    }
    return MS.STATE.runtimeObjectiveSmokes[missionId][objectiveId]
end

local function buildObjectiveAutoSmokeItems(objectiveDef)
    local items = {}
    local mon = objectiveDef.monitor or {}
    local smoke = getObjectiveSmokeConfig(objectiveDef)
    if not smoke then
        return items
    end

    local zoneColor = smoke.zoneColor or smoke.pickupZoneColor or smoke.defaultZoneColor or "green"
    local targetColor = smoke.targetColor or smoke.pickupTargetColor or smoke.defaultTargetColor or "white"
    local kind = tostring(mon.kind or ""):lower()

    local autoZone = smoke.autoZone
    local autoTarget = smoke.autoTarget
    if autoZone == nil then autoZone = true end
    if autoTarget == nil then autoTarget = true end

    if kind == "unit_alive_in_zone" or kind == "unit_reach_zone_alive" or kind == "unit_in_zone_alive" then
        if autoZone and (mon.zoneName or mon.point) then
            items[#items + 1] = {
                targetKind = "zone",
                zoneName = mon.zoneName,
                point = mon.point,
        ...
        if autoTarget then
            if mon.unitName then
                items[#items + 1] = {
                    targetKind = "unit",
                    unitName = mon.unitName,
                    color = targetColor
                }
            elseif mon.groupName then
                items[#items + 1] = {
                    targetKind = "group",
                    groupName = mon.groupName,
                    color = targetColor
    ...
    elseif kind == "group" and autoTarget and mon.groupName then
        items[#items + 1] = {
            targetKind = "group",
            groupName = mon.groupName,
            color = smoke.destroyColor or targetColor or "red"
        }
    elseif kind == "unit" and autoTarget and mon.unitName then
        items[#items + 1] = {
            targetKind = "unit",
            unitName = mon.unitName,
            color = smoke.destroyColor or targetColor or "red"
        }
    elseif (kind == "base_capture" or kind == "base_control" or kind == "airbase_control") and mon.baseName then
        if autoTarget then
            items[#items + 1] = {
                targetKind = "airbase",
                airbaseName = mon.baseName,
                color = smoke.captureColor or targetColor or "orange"
            }
        end
        if autoZone and (mon.zoneName or mon.point) then
            items[#items + 1] = {
                ...
                point = mon.point,
                radius = mon.radius,
                color = smoke.captureZoneColor or zoneColor or "orange"
            }
        end
    end
...
local function buildObjectiveSmokeItems(objectiveDef)
    local smoke = getObjectiveSmokeConfig(objectiveDef)
    if not smoke then
        return {}
    end
    ...
    local items = {}
    local autoItems = buildObjectiveAutoSmokeItems(objectiveDef)
    for i = 1, #autoItems do
        items[#items + 1] = autoItems[i]
    end

    for _, item in ipairs(smoke.items or {}) do
        items[#items + 1] = item
    end
...
local function resolveObjectiveSmokeTargetPoint(item)
    item = item or {}
    local targetKind = tostring(item.targetKind or ""):lower()

    if targetKind == "zone" or (targetKind == "" and (item.zoneName or item.point)) then
        local zone = resolveMonitorZone(item.zoneName, item.point, item.radius)
        if not zone then
            return nil
        end
        return {
    ...
    elseif targetKind == "unit" or (targetKind == "" and item.unitName) then
        return getUnitWorldPointByName(item.unitName)
    elseif targetKind == "group" or (targetKind == "" and item.groupName) then
        return getGroupWorldPointByName(item.groupName)
    elseif targetKind == "airbase" or targetKind == "base" or (targetKind == "" and (item.airbaseName or item.baseName)) then
        return getAirbaseWorldPointByName(item.airbaseName or item.baseName)
    elseif targetKind == "point" and type(item.point) == "table" then
        return {
            x = ensureNumber(item.point.x),
            y = ensureNumber(item.point.y),
            z = item.point.z ~= nil and ensureNumber(item.point.z) or ensureNumber(item.point.y)
        }
    end
...
local function refreshMissionObjectiveSmokes(def, mState, now)
    if MS.CONFIG.MISSION_SMOKE_ENABLED ~= true then
        return
    end

    if not def or not mState or mState.status ~= MS.STATUS.ACTIVE then
        MS.STATE.runtimeObjectiveSmokes[def and def.id or ""] = nil
        return
    end
    ...
    for _, objectiveDef in ipairs(def.secondaryObjectives or {}) do
        if objectiveDef.enabled ~= false then
            local smoke = getObjectiveSmokeConfig(objectiveDef)
            local objectiveId = tostring(objectiveDef.id or "OBJ")
            local oState = ensureSecondaryObjectiveState(mState, objectiveId)

            if not smoke then
                if MS.STATE.runtimeObjectiveSmokes[def.id] then
                    MS.STATE.runtimeObjectiveSmokes[def.id][objectiveId] = nil
                end
            else
                local stopOnPass = smoke.stopOnPass
                if stopOnPass == nil then
                    stopOnPass = MS.CONFIG.MISSION_SMOKE_DEFAULT_STOP_ON_PASS == true
                end

                if oState.completed and stopOnPass then
                    if MS.STATE.runtimeObjectiveSmokes[def.id] then
                        MS.STATE.runtimeObjectiveSmokes[def.id][objectiveId] = nil
                    end
                else
                    local state = getObjectiveSmokeState(def.id, objectiveId)
                    local refreshSeconds = ensureNumber(smoke.refreshSeconds or MS.CONFIG.MISSION_SMOKE_DEFAULT_REFRESH_SECONDS or 240)
                    if refreshSeconds <= 0 then
                        refreshSeconds = 240
                    end

                    if (now - ensureNumber(state.lastRefreshAt)) >= refreshSeconds then
                        local items = buildObjectiveSmokeItems(objectiveDef)
                        state.lastRefreshAt = now

                        for i = 1, #items do
                            local item = items[i]
                            local point = resolveObjectiveSmokeTargetPoint(item)
                            if point then
                                pcall(function()
                                    trigger.action.smoke(point, normalizeSmokeColorId(item.color or smoke.color or "white"))
                                end)
                            end
                        end
                    end
...
local function getObjectiveCleanupConfig(objectiveDef)
    local mon = objectiveDef and objectiveDef.monitor or nil
    local cfg = nil

    if mon and type(mon.cleanupOnPass) == "table" then
        cfg = mon.cleanupOnPass
    elseif type(objectiveDef and objectiveDef.cleanupOnPass or nil) == "table" then
        cfg = objectiveDef.cleanupOnPass
    end
...
local function getObjectiveCleanupTarget(objectiveDef, cfg)
    local mon = objectiveDef and objectiveDef.monitor or {}

    local kind = tostring(cfg and (cfg.targetKind or cfg.destroyTarget) or "auto"):lower()
    local unitName = cfg and (cfg.unitName or mon.unitName) or mon.unitName
    local groupName = cfg and (cfg.groupName or mon.groupName) or mon.groupName

    if kind == "" or kind == "auto" then
        if groupName and tostring(groupName) ~= "" then
            kind = "group"
        elseif unitName and tostring(unitName) ~= "" then
            kind = "unit"
        else
            kind = "none"
        end
...
local function destroyUnitByName(unitName)
    if not unitName or unitName == "" then
        return false, "unitName vacio"
    end

    local unit = Unit.getByName(unitName)
    if not unit then
        return false, "unit no encontrada"
    end
    ...
    if not okExists or not exists then
        return false, "unit no existe"
    end

    local okDestroy, err = pcall(function()
        unit:destroy()
    end)

    if okDestroy then
        return true, "unit destruida"
    end

    return false, err
end

local function destroyGroupByNameSafe(groupName)
    if not groupName or groupName == "" then
        return false, "groupName vacio"
    end

    local grp = groupExistsByName(groupName)
    if not grp then
        return false, "group no encontrado"
    end
    ...
    if okDestroy then
        return true, "group destruido"
    end

    return false, err
end

local function tryDestroyOwningGroupFromUnit(unitName)
    if not unitName or unitName == "" then
        return false, "unitName vacio"
    end
    ...
    local okGroup, grp = pcall(function()
        return unit:getGroup()
    end)

    if not okGroup or not grp then
        return false, "group de unit no encontrado"
    end
    ...
    if okDestroy then
        return true, "group padre destruido"
    end

    return false, err
end

local function executeObjectiveCleanup(def, objectiveDef, oState, now)
    if not def or not objectiveDef or not oState or oState.cleanupExecuted then
        return
    end

    local cfg = getObjectiveCleanupConfig(objectiveDef)
    if not cfg then
        return
    end
    ...
    local target = getObjectiveCleanupTarget(objectiveDef, cfg)
    local ok = false
    local result = "sin accion"

    if target.kind == "group" then
        ok, result = destroyGroupByNameSafe(target.groupName)

    elseif target.kind == "unit" then
        ok, result = destroyUnitByName(target.unitName)

        local fallbackToGroup = cfg.fallbackToGroup
        if fallbackToGroup == nil then
            fallbackToGroup = true
        end

        if (not ok) and fallbackToGroup then
            ok, result = tryDestroyOwningGroupFromUnit(target.unitName)
        end
    else
        result = "targetKind no soportado"
    end

    oState.cleanupExecuted = true
    oState.cleanupExecutedAt = now
    oState.cleanupResult = tostring(result or (ok and "OK" or "ERROR"))

    debugMissionLog(
        "Cleanup objetivo " .. tostring(def.id) ..
        " -> " .. tostring(objectiveDef.id) ..
...
local function refreshAllPendingObjectiveCleanups(now)
    now = now or timer.getTime()
        ...
        for _, objectiveDef in ipairs(def.secondaryObjectives or {}) do
            if objectiveDef.enabled ~= false then
                ...
                if cfg then
                    ...
                    if oState.completed and not oState.cleanupExecuted then
                        local delay = ensureNumber(cfg.delaySeconds or cfg.afterSeconds or 20)
                        if delay < 0 then
                            delay = 0
                        end

                        if not oState.cleanupDueAt then
                            local baseTime = ensureNumber(oState.completedAt)
                            if baseTime <= 0 then
                                baseTime = now
                            end
                            oState.cleanupDelaySeconds = delay
                            oState.cleanupDueAt = baseTime + delay
                            MS.STATE.dirty = true
                        end

                        if now >= ensureNumber(oState.cleanupDueAt) then
                            executeObjectiveCleanup(def, objectiveDef, oState, now)
                            MS.STATE.dirty = true
                        end
                    end
                end
...
local function buildSecondaryObjectiveDebugLines(def, mState, lines)
    for _, objectiveDef in ipairs(def.secondaryObjectives or {}) do
        if objectiveDef.enabled ~= false then
            ...
            elseif kind == "unit_alive_in_zone" or kind == "unit_reach_zone_alive" or kind == "unit_in_zone_alive" then
                local targetName = mon.groupName and tostring(mon.groupName) or tostring(mon.unitName)
                local targetType = mon.groupName and "group" or "unit"
                local extraCount = ""
                if mon.groupName then
                    extraCount =
                        " | aliveUnits=" .. formatDebugNumber(snap.aliveUnits) ..
                        " | inZoneAlive=" .. formatDebugNumber(snap.unitsInZoneAlive)
                end
                ...
                lines[#lines + 1] =
                    ...
                    " | nombre=" .. targetName ..
                    " | zona=" .. tostring(snap.zoneName or mon.zoneName or "PUNTO") ..
                    " | done=" .. boolText(oState.completed) ..
                    " | armed=" .. boolText(oState.armed) ..
                    " | pass=" .. boolText(oState.lastPass) ..
                    " | exists=" .. formatDebugNumber(oState.lastExists) ..
                    " | value=" .. formatDebugNumber(oState.lastValue) ..
                    " | alive=" .. formatDebugNumber(snap.alive) ..
                    " | inZone=" .. boolText(snap.inZone == true) ..
                    extraCount ..
                    " | zoneExists=" .. boolText(snap.zoneExists == true) ..
                    " | dist=" .. formatDebugNumber(snap.distance2D) ..
...
local function removeMissionMark(missionId)
    ...
    MS.STATE.runtimeObjectiveSmokes[missionId] = nil
end
...
local function getWarehouseDisplayLabel(itemDef)
    if type(itemDef) ~= "table" then
        return tostring(itemDef or "")
    end
    return tostring(itemDef.label or itemDef.drawLabel or itemDef.displayName or itemDef.itemName or "")
end
...
local function evaluateSecondaryObjectiveMonitor(mState, objectiveDef)
    ...
    elseif kind == "unit_alive_in_zone" or kind == "unit_reach_zone_alive" or kind == "unit_in_zone_alive" then
        local usingGroup = mon.groupName ~= nil and tostring(mon.groupName) ~= ""
        local data = nil
        local metricValue = 0

        if usingGroup then
            data = getGroupAliveInZoneMetrics(mon.groupName, mon.zoneName, mon.point, mon.radius, mon.groupMode)
            if tostring(mon.groupMode or "any"):lower() == "count" then
                metricValue = ensureNumber(data.unitsInZoneAlive)
            else
                metricValue = (data.alive > 0 and data.inZone == true) and 1 or 0
            end
        else
            data = getUnitAliveInZoneMetrics(mon.unitName, mon.zoneName, mon.point, mon.radius)
            metricValue = (data.alive > 0 and data.inZone == true) and 1 or 0
        end
        ...
        if not data.zoneExists then
            oState.debugStatus = "ZONA_NO_ENCONTRADA"
            oState.lastPass = false
            return false, metricValue, data
        end

        local pass = compareValues(metricValue, mon.op or "==", mon.value or 1)

        if pass then
            oState.debugStatus = "PASS"
        elseif data.alive <= 0 then
            oState.debugStatus = usingGroup and "GRUPO_NO_VIVO" or "UNIDAD_NO_VIVA"
        elseif data.inZone ~= true then
            oState.debugStatus = "FUERA_DE_ZONA"
        else
            oState.debugStatus = "PENDIENTE"
        end
        ...
        if objectiveDef.enabled ~= false then
            ...
            if not oState.completed then
                ...
                if pass then
                    ...
                    local cleanupCfg = getObjectiveCleanupConfig(objectiveDef)
                    if cleanupCfg and not oState.cleanupExecuted then
                        local delay = ensureNumber(cleanupCfg.delaySeconds or cleanupCfg.afterSeconds or 20)
                        if delay < 0 then
                            delay = 0
                        end
                        oState.cleanupDelaySeconds = delay
                        oState.cleanupDueAt = ensureNumber(oState.completedAt) + delay
                    end
...
local function mainLoop(_, now)
    ...
    if activeDef then
        ...
        if activeState.status == MS.STATUS.ACTIVE then
            ...
            refreshMissionObjectiveSmokes(activeDef, activeState, now)
    ...
    refreshAllPendingObjectiveCleanups(now)
