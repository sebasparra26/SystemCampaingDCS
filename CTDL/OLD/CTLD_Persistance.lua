...
CTDP.CONFIG = {
    ...
    -- Proteccion automatica para drones JTAC persistidos/reinyectados.
    -- Cuando CTLD_Persistance reconstruye un drone desde JSON usa mist.dynAdd()
    -- directamente, por eso estas propiedades deben aplicarse tambien aqui.
    PROTECT_JTAC_DRONES = true,
    PROTECT_JTAC_DRONE_RETRY_COUNT = 6,
    PROTECT_JTAC_DRONE_RETRY_INTERVAL = 2,

    -- Tipos tecnicos de drones que deben quedar protegidos al ser reinyectados.
    PROTECT_JTAC_DRONE_TYPES = {
        ["MQ-9 Reaper"] = true,
        ["RQ-1A Predator"] = true
    },
...
----------------------------------------------------------------
-- PROTECCION DE DRONES JTAC REINYECTADOS
----------------------------------------------------------------
local function isProtectedJtacDroneType(typeName)
    if not typeName then
        return false
    end
    ...
    for protectedType, enabled in pairs(CTDP.CONFIG.PROTECT_JTAC_DRONE_TYPES or {}) do
        if enabled then
            local p = lowerText(protectedType)
    ...
    -- Respaldo flexible por si DCS o CTLD guardan una variante del nombre.
    return string.find(t, "mq-9", 1, true) ~= nil
        or string.find(t, "reaper", 1, true) ~= nil
        or string.find(t, "rq-1", 1, true) ~= nil
        or string.find(t, "predator", 1, true) ~= nil
end

local function deploymentContainsProtectedJtacDrone(dep)
    if not dep then
        return false
    end

    if dep.crateUnit and isProtectedJtacDroneType(dep.crateUnit) then
        return true
    end

    for _, typeName in ipairs(dep.types or {}) do
        if isProtectedJtacDroneType(typeName) then
            return true
        end
    end
    ...
    if dep.groupData and dep.groupData.units then
        for _, unitData in ipairs(dep.groupData.units) do
            if isProtectedJtacDroneType(unitData.type) then
                return true
            end
        end
    end
...
local function applyProtectedJtacDroneCommands(dep, groupName, attempt)
    if CTDP.CONFIG.PROTECT_JTAC_DRONES == false then
        return false
    end

    if not deploymentContainsProtectedJtacDrone(dep) then
        return false
    end
    ...
    if not grp or not grp:isExist() then
        if attempt < (tonumber(CTDP.CONFIG.PROTECT_JTAC_DRONE_RETRY_COUNT) or 6) then
            timer.scheduleFunction(function()
                applyProtectedJtacDroneCommands(dep, groupName, attempt + 1)
                return nil
            end, nil, timer.getTime() + (tonumber(CTDP.CONFIG.PROTECT_JTAC_DRONE_RETRY_INTERVAL) or 2))
        end
    ...
    env.info(
        "[CTLD_PERSIST] Drone JTAC protegido: " ..
        tostring(groupName) ..
        " | invisible=true | immortal=true | unlimitedFuel=true"
    )

    if CTDP.CONFIG.DEBUG then
        trigger.action.outText(
            "[CTLD_PERSIST] Drone JTAC protegido: " .. tostring(groupName),
            8
        )
    end
...
local function restoreCtldRuntimeForDeployment(dep, groupName)
    ...
    -- La persistencia reinyecta con mist.dynAdd() directamente, no con
    -- ctld.spawnCrateGroup(). Por eso protegemos aqui los drones JTAC
    -- reconstruidos desde JSON.
    applyProtectedJtacDroneCommands(dep, groupName, 1)
